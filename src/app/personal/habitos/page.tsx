...
function HabitosContent() {
  ...
  return <>
    ...
    <div className="card table-card" style={{marginTop:18}}><table><thead><tr><th>Aluno</th><th>Hábito</th><th>Meta</th><th>Adesão</th></tr></thead><tbody>{habits.map(h=><tr key={h.id}><td>{studentName(h)}</td><td>{h.name}</td><td>{h.target}</td><td>{h.habit_logs?.length || 0} registros</td></tr>)}</tbody></table></div>
  </>;
}
