...
function TreinosContent() {
  ...
  async function load() {
    ...
    setWorkouts((w.data || []) as unknown as Workout[]);
  }
    ...
    <>
      ...
      <div className="card table-card" style={{ marginTop: 18 }}>
        <table><thead><tr><th>Treino</th><th>Aluno</th><th>Período</th><th>Status</th></tr></thead><tbody>
          {workouts.map(w => <tr key={w.id}><td><strong>{w.title}</strong></td><td>{w.students?.[0]?.full_name || '—'}</td><td>{w.start_date || '—'} até {w.end_date || '—'}</td><td>{w.status}</td></tr>)}
        </tbody></table>
