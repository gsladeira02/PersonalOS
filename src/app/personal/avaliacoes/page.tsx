...
function AvaliacoesContent() {
  ...
  return <>
    ...
    <div className="card table-card" style={{ marginTop: 18 }}><table><thead><tr><th>Aluno</th><th>Data</th><th>Peso</th><th>Medidas</th></tr></thead><tbody>{items.map(i => <tr key={i.id}><td>{studentName(i)}</td><td>{i.assessment_date}</td><td>{i.weight_kg || '—'} kg</td><td>Cintura {i.waist_cm || '—'} cm</td></tr>)}</tbody></table></div>
  </>;
}
