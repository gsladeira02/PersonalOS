...
3. Configure as variáveis de ambiente na Vercel usando `.env.example`. O `vercel.json` já define `npm install --no-audit --no-fund` para evitar travas de instalação.
...
- Esta versão corrigida remove o `package-lock.json` do pacote para evitar o erro de instalação `Exit handler never called` na Vercel.
